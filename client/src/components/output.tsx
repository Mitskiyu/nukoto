...
function Output() {
	...
	if (!hasImages) {
		return (
			<div className="w-full h-[360px] mx-auto sm:h-[632px] flex flex-col font-koden bg-surface0 outline-1 outline-surface1 rounded-sm">
				<h1 className="text-xl sm:text-2xl ml-2.5 sm:ml-3.5 mt-2 sm:mt-3 text-lavender">
					Output
				</h1>
	...
	return (
		<div className="w-full h-[360px] sm:h-[632px] flex flex-col font-koden text-text bg-surface0 outline-1 outline-surface1 rounded-sm">
			<div className="flex flex-row items-center">
				...
				<div className="mr-2.5 sm:mr-3.5 sm:text-lg text-sm flex w-full justify-end space-x-2 mt-2">
					{hasDone && (
						<button
							onClick={() => downloadAll()}
							className="cursor-pointer hover:bg-surface2 focus:outline-none sm:h-8 h-6 bg-surface1 text-lavender rounded-md text-center w-24 sm:w-36"
						>
							Download All
						</button>
					)}
					<button
						...
						className="cursor-pointer hover:bg-surface2 focus:outline-none sm:h-8 h-6 bg-surface1 text-lavender rounded-md text-center sm:w-24 w-16"
					>
						Convert
					</button>
					...
					<button
						...
						className="cursor-pointer hover:bg-surface2 focus:outline-none sm:h-8 h-6 bg-surface1 text-lavender rounded-md text-center sm:w-20 w-12"
					>
						Clear
					</button>
				</div>
					...
					<div key={index} className="w-full px-2.5 sm:px-3.5">
						<div className="w-full flex flex-row items-center">
							...
							<div className="flex flex-col ml-2 overflow-hidden flex-1">
								<p className="text-sm text-text sm:text-lg text-clip">
									{image.status !== "done"
										? image.file.name
										: `${image.file.name.split(".")[0]}.${image.convertedFormat?.toLowerCase()}`}
								</p>
								<div className="-mt-0.5 text-md text-subtext0">
							...
							<div className="flex items-center ml-auto">
								{image.status === "done" && (
									<button
										...
										className="p-1 mr-1 hover:bg-surface1 cursor-pointer rounded-sm"
									>
										<img
											className="w-4 h-4 sm:w-5 sm:h-5"
											src={cloudDownload}
								...
								<button
									onClick={() => useImageStore.getState().deleteImage(image.id)}
									className="p-1 hover:bg-surface1 rounded-sm cursor-pointer"
								>
									<img
										className="w-4 h-4 sm:w-5 sm:h-5"
										src={undo}
						...
						<hr className="border-1 mt-2 mb-2 rounded-sm border-surface1" />
					</div>
				))}
			</div>
		</div>
